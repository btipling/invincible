...
| `DATABASE_URL` | Vercel / local (optional tenancy) | Pooled Postgres — required with the two rows below for tenancy **on** |
| `AUTH_SECRET` | Vercel / local (optional tenancy) | Auth.js session secret |
| `CREDENTIALS_ENCRYPTION_KEY` | Vercel / local (optional tenancy) | Base64 32-byte AES-GCM KEK for sandbox tokens at rest |
| `SEED_ADMIN_EMAIL` / `SEED_ADMIN_PASSWORD` | Operator seed only | `npm run db:seed` bootstrap — never commit |
| `SEED_SANDBOX_URL` / `SEED_SANDBOX_TOKEN` | Operator seed only | Optional; else `SANDBOX_*` used at seed time |

**BYO:** set the table above on **your** Vercel project (and optional Actions secret on **your** repo). Tenancy: [docs/bring-your-own.md](docs/bring-your-own.md) §4a.  
...
Local: copy [`.env.example`](.env.example) → `.env.local` (Gateway key + optional `HARNESS_*` / `SANDBOX_*` / tenancy triple).
