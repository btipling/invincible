...
| **Optional** | Agent tools + sandbox | `POST /api/agent` — tenancy **off**: env BYO `SANDBOX_*`; tenancy **on**: DB grants + per-row `backend` (`byo`\|`vercel`) and image — [docs/sandbox.md](docs/sandbox.md); origin dogfood toolchain image: [dev/README.md](dev/README.md) |
