...
7. **Test DI/cost gate (automatic refuse):** production code is DI (`db` /
   `connect` / `lib/di` factories). Tests must **use those seams**. Slow work
   happens **once** or is **fully mocked** — not reconstructed test-after-test.
   Named case: `new PGlite()`. Same rule for `createDbConnection()`, real
   `hashPassword` in setup (except `password.test.ts`), live Redis/HTTP when a
   seam exists.
   ```bash
   gh pr diff <N> --repo btipling/invincible | rg -n '^\+.*(new PGlite\(|createDbConnection\()' || true
   gh pr diff <N> --name-only
   # changed *.test.ts: refuse if they add a real expensive constructor
   # instead of injected db/connect / the one shared helper (#431)
   ```
   **Refuse** if the PR **adds** a cold/slow constructor outside the one shared
   helper, repeats that cost per `it`/`beforeEach` when once-or-mock would do,
   or a **changed** test still constructs PGlite/`createDbConnection` after the
   helper exists. Do **not** refuse an unrelated PR solely for untouched legacy
   `lib/tenancy/*.test.ts` boots on `main` (`#431`). **Not waivable.**
   Adversarial-review must already have failed this class as Major / CONCERNS.
...
- **Test DI/cost gate failed** (PR adds a real expensive test constructor —
  `new PGlite(`, `createDbConnection(`, bcrypt in setup — or repeats that cost
  per test when a DI seam / shared helper / mock exists). Not waivable. See §3.7.
...
| Merging a PR that adds repeated slow test deps (PGlite, real bcrypt setup, live I/O) instead of DI/once/mock | Skill failure — §3.7 automatic refuse |
...
[ ] Test DI/cost gate: changed tests inject `db`/`connect` or mock; no added
    `new PGlite(` / `createDbConnection(` / bcrypt-in-setup outside the one
    helper or hasher unit test; slow work once-or-mocked (automatic refuse)
