...
describe('middleware auth gate', () => {
  ...
  it('401 JSON on unauth GET /api/personas when tenancy on', async () => {
    process.env.DATABASE_URL = 'postgres://x';
    process.env.AUTH_SECRET = 'test-secret-value-for-jwt-middleware!!';
    process.env.CREDENTIALS_ENCRYPTION_KEY = 'key-material';
    vi.resetModules();
    ...
    const { middleware } = await loadMw();
    const res = await middleware(
      new Request('http://localhost/api/personas', { method: 'GET' }) as never,
    );
    expect(res.status).toBe(401);
    const body = (await res.json()) as { error: string };
    expect(body.error).toBe(AUTH_REQUIRED_ERROR);
  ...
  it('allows GET /api/personas when JWT sub present', async () => {
    process.env.DATABASE_URL = 'postgres://x';
    process.env.AUTH_SECRET = 'test-secret-value-for-jwt-middleware!!';
    process.env.CREDENTIALS_ENCRYPTION_KEY = 'key-material';
    vi.resetModules();
  ...
  it('401 JSON on unauth POST /api/turns when tenancy on', async () => {
    process.env.DATABASE_URL = 'postgres://x';
    process.env.AUTH_SECRET = 'test-secret-value-for-jwt-middleware!!';
    process.env.CREDENTIALS_ENCRYPTION_KEY = 'key-material';
    vi.resetModules();
    ...
    const { middleware } = await loadMw();
    const res = await middleware(
      new Request('http://localhost/api/turns', { method: 'POST' }) as never,
    );
    expect(res.status).toBe(401);
    const body = (await res.json()) as { error: string };
    expect(body.error).toBe(AUTH_REQUIRED_ERROR);
  ...
  it('allows POST /api/turns when JWT sub present', async () => {
    process.env.DATABASE_URL = 'postgres://x';
    process.env.AUTH_SECRET = 'test-secret-value-for-jwt-middleware!!';
    process.env.CREDENTIALS_ENCRYPTION_KEY = 'key-material';
    vi.resetModules();
  ...
  it('/api/turns is on the middleware API-protected matcher (dual gate)', async () => {
    vi.resetModules();
    const { config } = await import('./middleware');
  ...
  it('401 JSON on unauth GET /api/settings/skills (collection) when tenancy on', async () => {
    process.env.DATABASE_URL = 'postgres://x';
    process.env.AUTH_SECRET = 'test-secret-value-for-jwt-middleware!!';
    process.env.CREDENTIALS_ENCRYPTION_KEY = 'key-material';
    vi.resetModules();
    ...
    const { middleware } = await loadMw();
    const res = await middleware(
      new Request('http://localhost/api/settings/skills', { method: 'GET' }) as never,
    );
    expect(res.status).toBe(401);
    const body = (await res.json()) as { error: string };
    expect(body.error).toBe(AUTH_REQUIRED_ERROR);
  ...
  it('401 JSON on unauth GET /api/settings/skills/:id/body when tenancy on', async () => {
    process.env.DATABASE_URL = 'postgres://x';
    process.env.AUTH_SECRET = 'test-secret-value-for-jwt-middleware!!';
    process.env.CREDENTIALS_ENCRYPTION_KEY = 'key-material';
    vi.resetModules();
    ...
    const { middleware } = await loadMw();
    const res = await middleware(
      new Request('http://localhost/api/settings/skills/skill-1/body', { method: 'GET' }) as never,
    );
    expect(res.status).toBe(401);
    const body = (await res.json()) as { error: string };
    expect(body.error).toBe(AUTH_REQUIRED_ERROR);
  ...
  it('allows GET /api/settings/skills when JWT sub present', async () => {
    process.env.DATABASE_URL = 'postgres://x';
    process.env.AUTH_SECRET = 'test-secret-value-for-jwt-middleware!!';
    process.env.CREDENTIALS_ENCRYPTION_KEY = 'key-material';
    vi.resetModules();
  ...
  it('allows GET /api/settings/skills/:id/body when JWT sub present', async () => {
    process.env.DATABASE_URL = 'postgres://x';
    process.env.AUTH_SECRET = 'test-secret-value-for-jwt-middleware!!';
    process.env.CREDENTIALS_ENCRYPTION_KEY = 'key-material';
    vi.resetModules();
  ...
  it('401 JSON on unauth GET /api/settings/personas/:id/versions when tenancy on', async () => {
    process.env.DATABASE_URL = 'postgres://x';
    process.env.AUTH_SECRET = 'test-secret-value-for-jwt-middleware!!';
    process.env.CREDENTIALS_ENCRYPTION_KEY = 'key-material';
    vi.resetModules();
    ...
    const { middleware } = await loadMw();
    const res = await middleware(
      new Request('http://localhost/api/settings/personas/pid/versions', { method: 'GET' }) as never,
    );
    expect(res.status).toBe(401);
    const body = (await res.json()) as { error: string };
    expect(body.error).toBe(AUTH_REQUIRED_ERROR);
  ...
  it('allows GET /api/settings/personas/:id/versions when JWT sub present', async () => {
    process.env.DATABASE_URL = 'postgres://x';
    process.env.AUTH_SECRET = 'test-secret-value-for-jwt-middleware!!';
    process.env.CREDENTIALS_ENCRYPTION_KEY = 'key-material';
    vi.resetModules();
  ...
  it('401 JSON on unauth POST /api/settings/personas/:id/rollback when tenancy on', async () => {
    process.env.DATABASE_URL = 'postgres://x';
    process.env.AUTH_SECRET = 'test-secret-value-for-jwt-middleware!!';
    process.env.CREDENTIALS_ENCRYPTION_KEY = 'key-material';
    vi.resetModules();
    ...
    const { middleware } = await loadMw();
    const res = await middleware(
      new Request('http://localhost/api/settings/personas/pid/rollback', { method: 'POST' }) as never,
    );
    expect(res.status).toBe(401);
    const body = (await res.json()) as { error: string };
    expect(body.error).toBe(AUTH_REQUIRED_ERROR);
  ...
  it('allows POST /api/settings/personas/:id/rollback when JWT sub present', async () => {
    process.env.DATABASE_URL = 'postgres://x';
    process.env.AUTH_SECRET = 'test-secret-value-for-jwt-middleware!!';
    process.env.CREDENTIALS_ENCRYPTION_KEY = 'key-material';
    vi.resetModules();
  ...
  it('/api/settings/personas is on the middleware API-protected matcher (dual gate)', async () => {
    vi.resetModules();
    const { config } = await import('./middleware');
    expect(config.matcher).toContain('/api/settings/personas/:path*');
    expect(config.matcher).toContain('/api/settings/personas');
  });
