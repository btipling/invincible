...
| Per-user agent skills (DB + API, phase 1 #498) | **Done** (code) | `user_skills` store + `GET /api/skills` summaries (no body) + server-side `getSkillBySlug` injection seam. Bodies are **non-secret** plaintext (no DEK); slug charset `^[a-z][a-z0-9_-]{0,63}$` (hyphen allowed) is the single source shared with the phase-3 slash parser. `/api/skills` sits on the **middleware auth edge** (`isApiProtected` + matcher) like `/api/personas` (dual gate: in-route `requireSessionUser` + middleware). Schema: GHA **`db-migrate`** (0011). Attach/injection + Wasm `Skill attached: <slug>` display shipped in phase 3 (#497): slash-command `/skill-name` + `/unskill`, session-sticky `meta.attachedSkills` (JSON-array-of-slugs reserved key, reused slug RE), bodies re-resolved each turn (staff-of-work; bodies stay server-only), bridge protocol **v12** (message kind **7** = `skill_attached`). Settings UI (phase 2) + skill-search tools (phase 4) ship separately. Ops: [docs/skills.md](docs/skills.md) |
...
| Cloud multi-device harness session (Redis multi-session, `/api/sessions*`, hybrid local+cloud) | `app/api/sessions/*`, `lib/sessionRepository.ts`, `lib/sessionCloudCaps.ts`, `lib/sessions/*`, `lib/tenancy/harnessSessionsRedis.ts`, `lib/tenancy/harnessSessions.ts` (archive read + shared validator), `lib/di/index.ts` (root), `app/harness/HarnessHost.tsx`, `middleware.ts`, [docs/session-model.md](docs/session-model.md), [SECURITY.md](SECURITY.md) — one-shot Postgres→Redis backfill: GHA **`sessions-redis-backfill`** (idempotent per-user marker); Postgres `harness_sessions` is a read-only archive. P1/GAP-1 (#452): session-carrier `meta.{logicalCwd,activeSandboxId}` folds into the PUT body and restores on pull/adopt |
| Harness stream chrome (Thinking collapse/caps, live tools) | `lib/harnessChat.ts`, `native/harness/src/ui.zig` (Thinking kind), protocol v12 in `lib/harnessBridge.ts` (Stop cancel v9; Thinking kind v8; tool-run kind 6 v10→live paint v11; skill-attach kind 7 v12) |
...
| User agent personas (backend + data, phase 1) | `db/schema.ts` `user_personas`, `db/migrations/0010_user_personas.sql`, `lib/tenancy/userPersonas.ts` (injected `db`/`connect`; **no DEK** — bodies are non-secret user content), `app/api/personas/route.ts` (GET summaries, no body), `lib/agent/agentBody.ts` (`parsePersonaId`), `lib/sessionCloudCaps.ts` / `lib/sessions/sessionStore.ts` (reserved `meta.{personaId,personaSnapshot}`, `PERSONA_SNAPSHOT_MAX_BYTES`=16 KiB, `HARNESS_SESSION_MAX_META_BYTES`=20 KiB), `middleware.ts` | |
| User agent skills (backend + data, phase 1) | `db/schema.ts` `user_skills`, `db/migrations/0011_user_skills.sql`, `lib/tenancy/userSkills.ts` (injected `db`/`connect`; **no DEK** — bodies are non-secret user content; slug RE `^[a-z][a-z0-9_-]{0,63}$` hyphen-allowed), `app/api/skills/route.ts` (GET summaries, no body), `lib/di/index.ts` (wire `createUserSkills`), `middleware.ts` (`isApiProtected` + matcher — `/api/skills` on the auth edge), `middleware.test.ts` | |
| User agent skills — attach/injection + Wasm display (phase 3 #497) | `lib/tenancy/skillInject.ts` (slash parse `/skill-name` + `/unskill`, `getSkillBySlug` resolve, session-sticky `meta.attachedSkills` re-resolve; fail-closed unknown/foreign, offline-safe store), `lib/sessions/sessionStore.ts` (`attachedSkills` reserved key + `validateMetaFields` JSON-array-of-slugs branch, reuse `SKILL_SLUG_RE`), `lib/tenancy/harnessSessions.ts` + `lib/sessionStore.ts` role `skill_attached`, `lib/agent/runAgent.ts` (`skillsPreamble` appended after persona), `app/api/agent/route.ts` (strip `/slug`, no-model `/unskill` turn, SSE `skill_attached` + JSON `skillEvents`/`attachedSkills`), `lib/agent/agentStream.ts` (`skill_attached` event), `lib/agentApi.ts` (client-safe `skillEvents`), `lib/harnessBridge.ts` (`MessageKind.SkillAttached = 7`, protocol **v12**), `lib/harnessChat.ts` (`roleToKind` + `skillRowText`/`pushSkillRow` display-only, JSON + SSE), `native/harness/src/{bridge,ui}.zig` + `rich/kinds.zig` (`skill_attached = 7`, `paintSkillAttached`), docs: [docs/skills.md](docs/skills.md) | |
