...
| Per-user agent skills (DB + API, phase 1 #498) | **Done** (code) | `user_skills` store + `GET /api/skills` summaries (no body) + server-side `getSkillBySlug` injection seam. Bodies are **non-secret** plaintext (no DEK); slug charset `^[a-z][a-z0-9_-]{0,63}$` (hyphen allowed) is the single source shared with the phase-3 slash parser. Schema: GHA **`db-migrate`** (0011). Attach/injection/UI/tool: later phases |
