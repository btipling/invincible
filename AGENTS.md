...
| API / AI Gateway / agent | `app/api/*`, `lib/agent/*`, `lib/sandbox/*`, `lib/gateway/modelCatalog.ts` (Gateway catalog, models.dev `vercel.models` fills holes; join adapts `max` → `xhigh` on the language-model `reasoning` wire; never fetch inside `'use step'` / `'use workflow'`) |
