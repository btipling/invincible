...
| Sandbox daemon | `sandbox/` | `exec` is argv-only (no shell) and the client's `/v1/exec` HTTP abort follows request `timeoutMs` + `EXEC_TIMEOUT_BUFFER_MS`, not a fixed 45 s ([docs/sandbox.md](docs/sandbox.md)) |
