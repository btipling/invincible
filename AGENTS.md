...
| Session/skill/meta caps (raise one) | `lib/sessionCloudCaps.ts`, `lib/tenancy/userSkills.ts`, `lib/sessions/sessionStore.ts`, `docs/harness-limits.md` (Session & skill caps) — caps default to generous; a plan introducing a cap MUST show it in its own table; plan-review never imposes/shrinks; raising the 256 KiB message cap ships only with the owning `ring_slot.zig`+`bridge.zig`+zig-test change. `REDIS_SAFE_OPAQUE_ID_MAX`/`RE` bump together in `lib/sessionCloudCaps.ts` |
