...
| Vercel Workflows turn owner (durable E path, plan #791 / source #768) | `lib/agent/turnWorkflow.ts` (+ `turnWorkerSeam.ts`, `turnWorkerPersist.ts`), `app/api/turns/route.ts`, `app/api/turns/[runId]/stream/route.ts`; probe fixture `lib/workflows/turnsFixtureWorkflow.ts` + `turnsFixtureEvents.ts` | **Slice E made the durable turn owner a Workflow**, building on the slice-B probe (plan #787). `POST /api/turns` now starts **`runTurnWorkflow`** — one prompt = one run, re-resolves grants/BYOK/sandbox/MCP/http per step via the seam, worker persists Blob transcript + envelope meta LWW, emits the same `AgentStreamEvent`s. `GET /api/turns/:runId/stream?startIndex=` resumes the run's stream (B reconnect primitive). **E is the durable owner for the E path only** — `/api/agent` is **not** cut over yet (F #769 owns the open-UI host/attach path; `/api/agent` stays the legacy tab-owned path). Durable attach is F (#769) / cancel is H (#771); the full turn-owner ownership table is slice I (#772). Fail-closed 503 on a Workflows-disabled start; **never falls back to `/api/agent`** |
...
- **Vercel Workflows** runs/steps live **server-side** (Vercel Functions +
  ...
  (`confirm=smoke`; always targets **production** — no preview env is built, and
  the target deployment is self-resolved from the Vercel API, so no
  `VERCEL_DEPLOYMENT_ID` is configured); an unready project fails
...
- **Verify:** dispatch Actions → **`workflows-smoke`** → Run workflow
  (`confirm=smoke`; **always targets production** — no preview env is built, and
  the target deployment is **self-resolved** from the Vercel API, so no
  `VERCEL_DEPLOYMENT_ID`) after deploying.
...
- **Durable turn owner (slice E, plan #791 / source #768):** the turn Workflow
  (`lib/agent/turnWorkflow.ts`) is the durable turn owner for the E path — worker
  persist (`turnWorkerPersist.ts`) + per-step re-resolve seam (`turnWorkerSeam.ts`)
  live server-side in the Workflows layer. Started via `POST /api/turns`; same
  `AgentStreamEvent`s; `/api/agent` not yet cut over (F owns the open-UI host
  path). Never a client/Wasm path; never a `/api/agent` fallback. Full ownership
  table is slice I (#772).
...
| Workflow turn owner (durable `/api/turns` prefix; slices B probe → **E real owner**) | `app/api/turns/route.ts`, `app/api/turns/[runId]/stream/route.ts`, `lib/agent/turnWorkflow.ts` (+ `turnWorkerSeam.ts`, `turnWorkerPersist.ts`), `lib/workflows/turnsFixtureWorkflow.ts` (+ `turnsFixtureEvents.ts`) | **Vercel backend owns this.** The slice-B probe became slice E's **real durable turn owner**: `POST /api/turns` starts `runTurnWorkflow` (server-side Workflow, worker Blob + envelope persist LWW, same `AgentStreamEvent`s); `GET /api/turns/:runId/stream?startIndex=` resumes the run. `/api/agent` is **not** cut over yet — F (#769) owns the open-UI host attach path, H (#771) owns cancel, full ownership table is slice I (#772). Never add a client/Wasm path to these routes |
