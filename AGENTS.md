...
| Durable turn-end (provider refusal is Error; caps are not) | `lib/agent/modelFinish.ts` (`isProviderRefusalFinish`: `content-filter` / `error` → SSE `content filtered` / `model error`; `length` is `done` with the partial text; `TURN_WALL_CLOCK_ERROR` / `TURN_WALL_CLOCK_WRAPUP` / `TURN_WALL_CLOCK_WRAPUP_SYSTEM`), `lib/workflows/turnLoop.ts` (persist `{ok:false}` never fails the turn; tool item failures continue to the next model round; 512-step cap → wrap-up then `done` + `reason:'steps'` (**wall wins if `deadlineAt` is already past, or if the steps wrap-up is aborted by the 1h signal**); **1-hour wall-clock cap → wall wrap-up (tools off) then SSE `error` `turn wall clock exceeded` + `reason:'wall'`** — plan #923), `lib/agent/turnPersistSeam.ts` (the worker **terminal** persist reconstructs the bound prior chain then suffix-merges this-run via `mergeCheckpointOntoPrior` into a flatten-root head (`prev`/`depth` omitted); after SSE `error` the host adopts that transcript for local paint only and skips the cloud PUT — plan #934; mid-turn `running` writes stay this-run-only), `lib/workflows/turnLog.ts` (`console.log` one JSON line `invincible.turn.model` / `invincible.turn.persist` inside `'use step'` — Observability → Workflows, **not** HTTP Runtime Logs; model line allowlists `reasoningChars?` + `completion?`; additive `invincible.turn.loop` line carries `status` + `reason` (`steps` \| `wall`) + bounded `elapsedMs`), `lib/harnessChat.ts` / `lib/agentSse.ts` (refusal `done` defense only), [docs/agent-stream.md](docs/agent-stream.md). There is **no** in-repo `maxOutputTokens`. |
...
| Durable-turn context compaction (A4 compaction engine — source #552) | `lib/agent/compaction.ts` (pure cut walk / checkpoint builder / honesty renderer — `Summary of earlier session (compacted, not live assistant prose):` user-role row; re-paired cut never splits an assistant tool call from its results) + `lib/agent/compactionBudget.ts` (`shouldCompact` on the **pre-trim** seed against the #944 fold budget — the reserve is already subtracted there and never subtracted twice), `app/api/turns/route.ts` (pre-start seed selection with the exact preference — honesty-prefixed live model-messages warehouse → checkpoint → ordinary model-messages → legacy `promptHistory`; every pointer confused-deputy `isObjectIdBoundTo` bound and re-validated/re-paired on read; trigger + `COMPACTION_*` rails; compact `start()` args are serializable-only), `lib/workflows/compactionStep.ts` (the one tools-off summarizer `'use step'` — same selected model, BYOK re-resolved in-step), `lib/workflows/turnLoop.ts` (failed/empty/unusable summary **fails open** to the ordinary bounded seed and never persists a fabricated checkpoint; `'cancelled'` / `'wall_clock'` skip the first tools-on round — Stop / 1h cap still terminate), `lib/agent/turnPersistSeam.ts` (terminal persist writes the checkpoint as its **own session-bound Blob object** + advances `meta.compactionPointer`; persist-time `boundCheckpointForPersist` drop-oldest re-rails `retainedTail` (keep newest = this turn) under `COMPACTION_CHECKPOINT_MAX_BYTES`; a still-over body fails closed `compaction_write_failed` — never a silent `writeSegment` clip), `lib/sessions/sessionStore.ts` (reserved `compactionPointer` key — worker-authored, drop-to-unset, envelope PUT copy-forward on host omit; durable, no per-turn volatility rewrite). **Logical, not physical:** compaction changes the next model seed only — transcript and earlier model-messages Blob objects stay retained. Caps: six existing values in `lib/sessionCloudCaps.ts` (reserve 16 384 tokens, summary 8 000 chars, files-touched 256, checkpoint Blob 2.25 MiB, summarizer span 2 MiB, combined compact `start()` args 3 MiB) — see [docs/harness-limits.md](docs/harness-limits.md) · Compaction limits. docs: [docs/session-model.md](docs/session-model.md) · Compaction, [docs/feature-divide.md](docs/feature-divide.md) |
...
| Per-turn freshness reminder (plan #941, source #693 — the prompt-side sentence of the #693 law: persisted `tool_result` snapshots are NOT live file views) | `lib/agent/freshnessReminder.ts` (pure projection: `buildFreshnessReminder` pairs committed ok `read_file` tool rows with their assistant `toolCalls` by `toolCallId` → `{paths}`; `renderFreshnessReminder` → the locked `Error: File-freshness law…` volatile copy with the **full-read rule** + #563 `limit>=totalLines` escape hint; `serializeFreshnessReminder` → the bounded `{paths}` body; TextEncoder byte math — NO Node `Buffer`, the #939 canvas lesson), `lib/sessionCloudCaps.ts` (`FRESHNESS_REMINDER_MAX_PATHS` = 64 drop-oldest / `FRESHNESS_REMINDER_MAX_BYTES` = 16 KiB — NEW caps, no existing cap touched), `lib/agent/turnPersistSeam.ts` (terminal persist writes the `{paths}` JSON as its **own Blob object** + sets `meta.freshnessReminderPointer`, fail-closed `freshness_reminder_write_failed`; **volatility — rewritten on EVERY persist, a zero-read turn writes `{paths:[]}`**), `lib/sessions/sessionStore.ts` + `lib/agent/workerMetaOverlay.ts` (reserved `freshnessReminderPointer` key + drop-to-unset + `copyForwardFreshnessReminderPointer` in `upsertEnvelope`), `lib/workflows/turnLoop.ts` (`derivePersistFold` derives the `freshnessReminder` sibling; loop passes the pointer to `deps.modelStep` on the **FIRST round only**), `lib/workflows/turnWorkflow.ts` + `lib/workflows/modelGenerateStep.ts` + `lib/workflows/inStepPreambles.ts` (`resolveInStepFreshnessReminder` — in-step read via the envelope/blob seams, `isObjectIdBoundTo` fail-closed, fail-open; folds the reminder as the trailing `{role:'error'}` row BEFORE `toModelMessages`), `app/api/turns/route.ts` (sanitize-only pointer pass-through in `start()` args — the route NEVER fetches the Blob), `lib/sessionRepository.ts` (host `cloudMetaFor` **never emits** the key — a stale snapshot PUT would LWW-stomp the worker pointer). Names only — never mtimes/sizes/hashes/bodies. docs: [docs/session-model.md](docs/session-model.md), [docs/agent-stream.md](docs/agent-stream.md), [docs/harness-limits.md](docs/harness-limits.md) |
...
| User agent skills — attach/injection + Wasm display (phase 2 #517; catalog inject plan #557/#931) | `lib/tenancy/skillInject.ts` (Production `/harness` re-resolves sticky ∪ always-on only — durable step `command: none`, no slash parse; slash `/skill-name` + `/unskill` is legacy `/api/agent` tests/JSON only; happy-path attach existence via `listUserSkillsBySlugs` summary (same projection as `listUserSkills`; never `getSkillBySlug`), list fail-open `skillExistsBySlugs`, session-sticky `meta.attachedSkills` re-resolve; the inject is a bounded **catalog** from candidate-scoped summaries (slug + name + description, never bodies — bodies ride on-demand `fetch_skill`; 256 KiB `HARNESS_SESSION_MAX_ATTACHED_BODY_BYTES` is the ceiling, not the default inject; attach is no longer size-rejected); fail-closed unknown/foreign, offline-safe store), `lib/tenancy/harnessSessions.ts` + `lib/sessionStore.ts` role `skill_attached`, `lib/agent/runAgent.ts` (`skillsPreamble` appended after persona), `app/api/agent/route.ts` (strip `/slug`, no-model `/unskill` turn, SSE `skill_attached` sticky persist set matching JSON `attachedSkills` — always-on stripped), `lib/agent/agentStream.ts` (`skill_attached` event), `lib/agentApi.ts` (client-safe `skillEvents`), `lib/harnessBridge.ts` (`MessageKind.SkillAttached = 7`, protocol **v12**), `lib/harnessChat.ts` (`roleToKind` + `skillRowText`/`pushSkillRow` display-only, JSON + SSE), `native/harness/src/{bridge,ui}.zig` + `rich/kinds.zig` (`skill_attached = 7`, `paintSkillAttached`), docs: [docs/skills.md](docs/skills.md), [docs/session-model.md](docs/session-model.md) | |
...
| Harness model catalog (protocol v3) + **selected-model persistence (protocol v16)** + **reasoning-effort picker (protocol v23)** + **resolved-provider label (protocol v24)** | `lib/harnessBridge.ts`, `native/harness/src/bridge.zig` (`inv_set_selected_model` restore-by-id + `inv_has_pending_model_change`/`inv_ack_pending_model_change` for a user Next cycle; v23 `inv_clear_reasoning_efforts` / `inv_push_reasoning_effort` / `inv_set_selected_reasoning` + pending-reasoning-change; **v24** `inv_set_resolved_provider`), `native/harness/src/{reasoning_picker,reasoning_catalog,model_picker}.zig`, `lib/agent/resolvedProvider.ts` (client-safe extract + `formatResolvedProviderLabel`; **do not** import `lib/gateway/byokProviders.ts`), `lib/agent/workerMetaOverlay.ts` (`WORKER_META_KEYS` += `resolvedProvider`), `app/harness/HarnessHost.tsx` (boot/adopt/switch restore, New/Clear reset, live POST `reasoning`), `lib/sessionStore.ts` + `lib/sessionRepository.ts` + `lib/sessions/sessionStore.ts` (reserved `meta.selectedModel` + `meta.reasoningEffort` + worker-owned `meta.resolvedProvider`; `sanitizeModelId` ≤ 128 B / `sanitizeReasoningEffort` ≤ 32 B / `sanitizeResolvedProvider` ≤ 32 B in `lib/sessionCloudCaps.ts`, Zig `MAX_REASONING_EFFORT_LEN` / `MAX_RESOLVED_PROVIDER_LEN` parity-locked), [docs/session-model.md](docs/session-model.md), [docs/feature-divide.md](docs/feature-divide.md) |
