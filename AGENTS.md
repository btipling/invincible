...
| Harness stream chrome (Thinking collapse/caps, live tools) | `lib/harnessChat.ts`, `native/harness/src/ui.zig` (Thinking kind), protocol **v14** in `lib/harnessBridge.ts` (Stop cancel v9; Thinking kind v8; tool-run kind 6 v10→live paint v11; skill-attach kind 7 v12; **status-slot store v13** — `inv_set_status_slot` / `inv_status_slot_len/copy` / `inv_status_slots_clear`; **turn-clock feed v14** — `inv_set_turn_elapsed`) |
