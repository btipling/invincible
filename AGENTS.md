...
| Tool-run aggregation + expandable transcript control (#325) | `lib/toolRun.ts` (encode/decode, host aggregation), `lib/harnessChat.ts` (stream/JSON aggregation → kind 6 `tool_run`), `lib/sessionStore.ts` role `tool_run`, `native/harness/src/rich/toolrun.zig` (decode), `native/harness/src/ui.zig` (`paintToolRun`), protocol **v10**; expand state + stick-to-bottom reuse dvui `reorder_tree.zig` / `scrolling.zig` idioms |
