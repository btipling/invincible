...
| Agent SSE stream (tools + text + reasoning) | `lib/agent/agentStream.ts`, `lib/agent/runAgent.ts`, `lib/agent/reasoningConfig.ts`, `lib/agent/generateOneRound.ts`, `lib/workflows/modelGenerateStep.ts`, `lib/workflows/turnSseWrite.ts` (durable live write of `reasoning_delta` / `text_delta` / `tool_start`; loop does not dump those events), `app/api/agent/route.ts`, `lib/agentApi.ts`, `docs/agent-stream.md` |
