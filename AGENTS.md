...
| Workspace status bar (protocol v13, plan #538/#541) | `native/harness/src/{bridge,ui}.zig` (status-slot store + header `paintStatusSlots` right-aligned slot pack; sandbox/cwd slots now, git/context reserved), `lib/harnessBridge.ts` (`StatusSlot`, `setStatusSlot`/`getStatusSlot`/`clearStatusSlot`/`clearStatusSlots`, `STATUS_SLOT_MAX_BYTES` mirror), `lib/harnessChat.ts` (`foldStatusSlots` — folds `activeSandboxId` + `cwd` after hydrate and after **every** agent turn, success **and** fail (403-clear / committed `change_dir` repaint the pack — PR #543), host-ellipsized to the byte cap before the wire), `app/harness/HarnessHost.tsx` (Clear/New clears the pack), `lib/sessionCloudCaps.ts` (`STATUS_SLOT_MAX_BYTES` = 96 — client-safe single source), docs: [docs/feature-divide.md](docs/feature-divide.md), [docs/harness-limits.md](docs/harness-limits.md), [docs/session-model.md](docs/session-model.md) | |
