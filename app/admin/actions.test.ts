...
describe('admin inference actions authz', () => {
  ...
  it('disableProviderSecretAction rejects non-admin', async () => {
    ...
    const { disableProviderSecretAction } = await import('./actions');
    const fd = new FormData();
    fd.set('secretId', 'sec-1');
    const r = await disableProviderSecretAction({}, fd);
    expect(r.error).toMatch(/Admin access/);
  ...
  it('updateProviderSecretAction passes session tenantId', async () => {
    ...
    const { updateProviderSecretAction } = await import('./actions');
    const fd = new FormData();
    fd.set('secretId', 'sec-1');
    fd.set('name', 'renamed');
    const r = await updateProviderSecretAction({}, fd);
