...
describe('GET /api/skills', () => {
  ...
  function mockList(
    listUserSkills: (userId: string) => Promise<unknown> = vi.fn(async () => ({
      ok: true,
      value: [summaryA],
    })),
  ) {
  ...
  it('returns skill summaries WITHOUT body', async () => {
    vi.resetModules();
    ...
    expect(body.skills).toEqual([summaryA]);
    expect(JSON.stringify(body)).not.toContain('body');
    expect(JSON.stringify(body)).not.toContain('secret');
  });

  it('empty skill list → []', async () => {
    vi.resetModules();
    mockSession({ ok: true, user: { id: 'u1' } });
