...
export async function POST(req: Request): Promise<Response> {
  ...
  // Phase 3 (#497) — leading-slash skill commands. Parse the command, strip the
  ...
  ): import('../../../lib/agent/agentStream').AgentStreamEvent => ({
    type: 'skill_attached',
    slug: e.slug,
    action: e.action,
    ok: e.ok,
  ...
  try {
    ...
    // Phase 3 (#497) — resolve attached skills (sticky re-read from
    ...
    // A pure `/slug` attach with no remaining prose, or a `/unskill` detach,
    // is a NO-MODEL turn: emit the display-only `skill_attached` rows + a short
    ...
    if (stream) {
      ...
      const bodyStream = new ReadableStream<Uint8Array>({
        async start(controller) {
          ...
          // Phase 3 (#497): emit the display-only `skill_attached` events at the
