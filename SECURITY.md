...
| Cross-user isolation | Tenant via `loadSoleMembership` (server); other-user id → **404**; ids/fn restricted to Redis-safe `^[A-Za-z0-9_-]{1,512}$` so no glob/key bleed |
...
| Caps (abuse / size) | No message-count cap; ≤**262 144** UTF-8 bytes per message text; Function-carried full-record body ≤**2 MiB** (`HARNESS_SESSION_MAX_FUNCTION_BODY_BYTES` — the primary wire bound; never a one-shot >4.5 MB Function payload), with an **8 MiB** Blob transcript-**object** ceiling **only** on the client→Blob path (`HARNESS_SESSION_MAX_BODY_BYTES`); record id ≤**512**; `meta` is schema-typed reserved (title/legacySnapshotId/logicalCwd/activeSandboxId/personaId/personaSnapshot/transcriptPointer/checkpointPointer/modelMessagesPointer/attachedSkills/selectedModel/usage) + **1 MiB** serialized size cap |
| Blob contents | Transcript objects: message roles/text/ids/timestamps. Sibling **model-messages** objects (`meta.modelMessagesPointer`, plan #936): typed `user` / `assistant`(+`tool-call`) / truncated `tool-result` rows. Reserved `meta` scalars only — **never** Gateway keys, sandbox tokens, MCP secrets, PATs, or host absolute paths |
| Transcript object security (phase 0 #515) | Transcript object ids are Redis-safe opaque (`meta.transcriptPointer`); the model-messages seed is a **sibling** session-bound Blob (`meta.modelMessagesPointer`, same `isObjectIdBoundTo` confused-deputy gate on envelope PUT and `POST /api/turns` read). Envelope/object read trust-but-verifies (identity mismatch fails closed); minted upload URL is **short-lived, scoped, credential-checked** (never a static token in client/Wasm); full-record `GET` stays only for legacy roll-forward while those blobs remain small |
...
(`meta.personaSnapshot`, ≤ 512 KiB). Nothing secret is added to the client bundle
