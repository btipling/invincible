...
| `/health` discloses the jail root | Since daemon v2, `GET /health` returns the **low-sensitivity** `workspaceRoot` (per-binding jail root) **without** bearer auth, alongside `version`/`daemonVersion`. It is reachable only on the token-private daemon port and is already visible to any bearer-token holder via `exec pwd`, so this is not a new secret class. All FS mutation stays `/v1/*` token-gated; keep other host paths/IDs private and do **not** expose the daemon port publicly |
