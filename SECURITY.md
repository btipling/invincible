...
| Residual (platform) | Invincible does **not** fall back to host env-model routing under tenancy on. Vercel AI Gateway remains a third party: misconfigured BYOK can still error at the platform; mitigate with always-send BYOK, `only: [provider]`, surface errors, redact secret material from error JSON |
