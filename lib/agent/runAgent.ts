...
export const DEFAULT_AGENT_SYSTEM = [
  ...
  'Must read_file a path in this agent run before str_replace or overwriting an existing file with write_file (with offset=1 covering every line of the returned content, not truncated by limit or maxBytes). Creating a new file with write_file does not require a prior read. If tools report the file changed since your last read (another edit, command, concurrent session, or device on the same sandbox), read_file again before editing.',
