...
export type RunAgentParams = {
  ...
  /**
   * Skills preamble (phase 3, #497). Server-resolved attached-skill bodies,
   * appended after the persona preamble so the skill is the final standing-order
   * block the model sees. Skills are staff-of-work (re-resolved each turn), so
   * this is NOT a locked snapshot. Empty/whitespace is dropped.
   */
...
function resolveSystem(
  ...
  // Skills preamble (phase 3, #497) appends last — after even the persona — so
