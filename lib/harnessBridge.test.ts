...
describe('lifecycleName / messageKindLabel', () => {
  it('maps enums', () => {
    ...
    expect(messageKindLabel(MessageKind.ToolRun)).toBe('tools');
  });
});
...
describe('reasoning efforts (protocol v22)', () => {
  ...
  it('v22 exports are REQUIRED (fail-closed when any missing)', () => {
    const exp = makeMockExports() as unknown as WebAssembly.Exports;
    expect(isHarnessBridgeExports(exp)).toBe(true);
    for (const name of [
      'inv_clear_reasoning_efforts',
...
describe('skill_attached kind (protocol v12)', () => {
  it('MessageKind.SkillAttached is the next enum value 7 (never the protocol version)', () => {
    ...
    expect(HARNESS_PROTOCOL_VERSION).toBe(22);
  });
