...
/**
 * Reject raw PUT bodies larger than this (generous 8 MiB transcript-object
 * ceiling). Phase 0 (#515) moved the transcript to Blob objects ferried by
 * client→Blob uploads, so this caps an object, not a one-shot Function body —
 * the Vercel Function 4.5 MB request/response ceiling is the inviolable wire
 * bound and does not apply to Blob objects. A function-carried write/read must
 * only ever carry a per-wire body under that 4.5 MB limit (the envelope/meta
 * path below).
 */
