/**
 * Skill attachment resolver (parent #495 / phase 3 #497).
 *
 ...
 *                      not-attached); the whole line is consumed (never folded to
 *                      the model prompt).
 *
 * Unlike the locked persona (`meta.personaSnapshot`), skills are **not**
 * snapshotted into `meta` — we store **slugs only** and re-resolve their bodies
 * from the store each turn. So a mid-session skill edit applies next turn
 * (skills are staff-of-work, not identity). `meta` stays small (slugs ≪ 20 KiB).
 *
...
export const SKILL_COMMAND_RE = /^\/([a-z][a-z0-9_-]{0,63})(?:\s|$)/;
/** Detach slash command: `/unskill slug`. Must be checked BEFORE the attach RE. */
export const UNSKILL_COMMAND_RE = /^\/unskill\s+([a-z][a-z0-9_-]{0,63})(?:\s|$)/;
