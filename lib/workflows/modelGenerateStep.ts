 ...
 * Do NOT export non-step helpers from this file — workflow-mode SWC keeps
 * exported functions in the canvas bundle (prod deploy of #940). Preamble
 * resolution lives in `inStepPreambles.ts` and is dynamically imported below.
 */
...
export async function modelGenerateStep(
  ...
  // Dynamic import INSIDE the `'use step'` body so workflow-mode SWC stubs
  // it away. Do NOT statically import `./inStepPreambles` from this file —
  // that helper's `harnessSessionsRedis` / `sessionStore` graph is illegal in
  // the workflow VM (prod deploy of #940).
  const { resolveInStepPreambles } = await import('./inStepPreambles');
