...
| Cloud API | Redis multi-session via `GET`/`POST` `/api/sessions` + `GET`/`PUT`/`DELETE` `/api/sessions/:id` (path-`:id` write key) — auth required; fail-closed with **401** when unauth |
