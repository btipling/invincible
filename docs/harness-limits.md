...
| Blockquote | GFM-style `>` lines (optional ≤3 lead spaces). Nested `>>`… depth 1–6 with indent + **TEAL border bar**. **Strict lines** (no lazy continuation). zmd has no native quote — pre-partition + strip, then same sugar/inline pipeline. |
