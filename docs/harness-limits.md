...
| Height budget | Every frame: dvui flex layout — transcript `expand=.both` (takes remaining space), composer natural height with `max_size_content` cap (124 px), status bar absolute-rect at the very bottom. The status bar margin (`STATUS_BAR_H` = 64 px) is reserved below the composer so the transcript never paints behind it. Tall content cannot push chrome off-canvas — the composer is capped and the transcript scrolls internally |
| Wrap / grow | Field is **`break_lines`** + grows **vertically** with wrapped lines up to `COMPOSER_INPUT_MAX_H` (120 px) then scrolls **inside** the entry. Composer chrome box caps at `COMPOSER_INPUT_MAX_H + 2*COMPOSER_HUG_PAD` (124 px) via `max_size_content`. Never a horizontal gutter (repo no-h-scroll policy, #344/#457/#579) |
