...
| Detail vs scroll | Level-2 detail paints **inline** inside the one outer transcript scroller — there is **no** nested `scrollArea`. Command/output previews use the embedded **Vera Sans Mono** face (`exec`/filesystem/`http_*`); prose/other detail stays the body face. Long detail is bounded by the server preview cap (`TOOL_RUN_PREVIEW_MAX_CHARS` = 100k, head **40** / tail **10** lines + `… (M more lines)`) + the 200-item group cap, so the transcript wheel is not trapped |
...
| Group boundaries | A new group starts on user send, on a real (non-empty) assistant segment, and at turn end. Thinking/reasoning between tools is **not** a group boundary — one row per uninterrupted streak. A group's counts are written **once** at the boundary (commit-once), not streamed live via `update_last` |
