...
| Band | Behavior |
|------|----------|
| **Transcript** | One outer full-bleed `scrollArea` from the canvas top to the composer (no pane pad, no card fill/border). The auto vertical bar sits on the **canvas right edge** |
...
| Height budget | Every frame: absolute-rect bands — transcript is **full-bleed** from the canvas top to the composer (height = viewport − dynamic composer `composer_h` − status bar `STATUS_BAR_H`; no inter-band gap), composer sits above the status bar with dynamic height from previous-frame measurement (idle ~44 px, max 124 px), status bar absolute-rect flush to the canvas bottom. Both transcript and composer use `Options.rect` so neither participates in root flex; the scrollArea's `.auto` bar cannot publish virtual content height as min-size (dvui `ScrollContainerWidget.deinit` overwrites `min_size.h` with full content — adversarial review #584 Blocker L1). Tall content cannot push chrome off-canvas |
