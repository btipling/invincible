...
| str_replace diff | Successful `str_replace` appends a `-`/`+` diff block (old→new) to the result text so the L2 expander shows what was matched and replaced (plan #663). Old/new strings capped at **4 096 bytes each** (with explicit `…` when truncated) → diff block ≤ ~8 KiB, well under the preview caps. Per-line `-`/`+` prefixes; `replace_all` shows one pair (all replacements identical). Error results keep the short one-liner (no diff block). Redaction: `redactSecrets` runs over the full header + diff block in `finalize`, so tokens/keys in old/new are redacted in the diff | `lib/agent/tools.ts` (inline in `str_replace.execute`) |
