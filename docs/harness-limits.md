...
| **Composer chrome** | Single-row text field + one trailing **icon-only** button (▶ Send / ■ Stop); idle height hugs one line (~44 px), grows up to `COMPOSER_INPUT_MAX_H + 2*COMPOSER_HUG_PAD` (124 px chrome) then scrolls internally (plan #579, revises #457 fixed band) |
...
| Height budget | Every frame: absolute-rect bands — transcript at the top (height = viewport − composer `COMPOSER_MAX_CHROME_H` · status bar `STATUS_BAR_H` · gaps), composer fixed 124 px above the status bar, status bar absolute-rect at the very bottom. Both transcript and composer use `Options.rect` so neither participates in root flex; the scrollArea's `.auto` bar cannot publish virtual content height as min-size (dvui `ScrollContainerWidget.deinit` overwrites `min_size.h` with full content — adversarial review #584 Blocker L1). Tall content cannot push chrome off-canvas |
| Wrap / grow | Field is **`break_lines`** + grows **vertically** with wrapped lines up to `COMPOSER_INPUT_MAX_H` (120 px) then scrolls **inside** the entry. Composer chrome box is a fixed 124 px absolute rect (`COMPOSER_MAX_CHROME_H` = `COMPOSER_INPUT_MAX_H + 2*COMPOSER_HUG_PAD`); the textEntry hugs one line when idle (~44 px), grows up when multi-line, and scrolls internally past 120 px. Never a horizontal gutter (repo no-h-scroll policy, #344/#457/#579) |
