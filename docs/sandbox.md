...
| `POST` | `/v1/read_file` | Bearer | Read file (max 16 MiB prefix from byte 0); additive `mtimeMs` + `size` when daemon supports. The **agent** `read_file` tool then windows that prefix: `offset` (1-based line, default 1) + `limit` (default 1000) return `N→` numbered lines; when the window clips the file (more lines remain beyond `limit` and not byte-truncated) the tool appends a continuation hint, `[File continues beyond line N — next..end: use offset=N+1 to read the remaining M lines]`. Daemon request shape is still `path` + `maxBytes` |
...
| Truncated read | Does **not** authorize edit — including a default 1000-line window on a longer file, any `offset` other than 1, or a `maxBytes` prefix. Re-read the full file (`offset` 1 and a `limit` that covers every line) first. When only lines were clipped (not `maxBytes`), the tool appends a `[File continues beyond line N — …]` hint naming the `offset`+`limit` to read the remainder |
