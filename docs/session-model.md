...
| `reasoningEffort` | **Optional** selected reasoning-effort token (Gateway language-model wire value, e.g. `low` / `high`). **Session-owned** (plan #898): synced as reserved `meta.reasoningEffort`. Restore-by-value after the host pushes this model's joined list (protocol **v23**). Joined options omit tokens the language-model API rejects (`max` today — dropped, not aliased to `xhigh`). Poison / not-in-list / unset (including a stored `max`) → `defaultEffortFromOptions` (never auto `max`). `sanitizeReasoningEffort` (`^[a-z0-9_-]{1,32}$`) drops charset poison to unset — never 400. Submit reads live `getSelectedReasoning()` |
