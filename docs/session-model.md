...
| Piece | Behavior |
|-------|----------|
| Checkpoint carrier | `meta.compactionPointer` holds only a Redis-safe, session-bound object id. Its Blob object is the typed checkpoint `{ summary, filesTouched, retainedTail }` (≤ `COMPACTION_CHECKPOINT_MAX_BYTES` = 2.25 MiB), written by the terminal persist seam after a successful compaction — never envelope `meta`, never a Function body. `retainedTail` is the persist-time live post-compact warehouse (this turn included; `livePostCompactTail`), not the frozen cut-time tail, then `boundCheckpointForPersist` drop-oldest re-rails that tail to `COMPACTION_CHECKPOINT_MAX_BYTES` (newest / this turn kept) so the Blob write succeeds rather than fail-closing onto the prior pointer. Honesty-prefixed live model-messages remains the primary seed; the checkpoint fallback may drop older post-compact rows when the live warehouse plus this turn exceeds the Blob ceiling. The host flatten PUT never emits the key, and `upsertEnvelope` copy-forwards the stored valid value; a later successful compaction advances it. |
