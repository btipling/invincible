...
| `turnRunId` | **Optional** durable turn **run id** (Redis-safe opaque, `^[A-Za-z0-9_-]{1,512}$`). **Session-owned** (plan #789 / source #766, backend-agents slice C): synced to the cloud record as the reserved `meta.turnRunId`. Names **at most one** live run behind the session. **Absent = no durable run / idle** (today's Ready). The host **detach** seam keys on its **presence**: a durable run (id present) is owned server-side and survives the tab — a viewport close only **detaches** (closes the reader), it never aborts; a legacy turn with **no** id is still the tab-owned `/api/agent` fetch and **does** abort on tear-down so nothing burns unpersisted. `sanitizeTurnRunId` (wraps the Redis-safe opaque rule) drops a poisoned id to unset — never a 400. **Plan #789 (source #766, slice C) shipped the carrier; plan #791 (slice E) populates it** — the `POST /api/turns` start surface persists `meta.turnRunId = run.runId` + `turnStatus='running'` when it starts the turn Workflow ([feature-divide.md](feature-divide.md)), and the worker persist step closes it on completion. `getRun(runId)` is the source of truth |
| `turnStatus` | **Optional cached hint** of the turn status (`idle` \| `running` \| `cancelling`). **Hint only** — `getRun(runId)` is the authority when a `turnRunId` is present (the tab can detach and nothing polls it). Rides reserved `meta.turnStatus`; `sanitizeTurnStatus` (exact enum accept, ≤ `TURN_STATUS_MAX_BYTES` = 32) drops a poisoned value to unset, never a 400. Absent = unclear/clear. Slice E sets it to `running` on start and reconciles on completion |
...
- **Worker (turn Workflow) Blob writer — slice E (#791).** When a run is detached
  (or the host never attached), the **server-owned turn Workflow** is an additional Blob
  writer: the worker persist step mints + PUTs append-only transcript segments
  (`lib/agent/turnWorkerPersist.ts`) and advances `meta.transcriptPointer` **only on a
  successful PUT** (LWW with the host via `updatedAt`), so a detached durable run still
  saves. It overlays only the worker-owned envelope `meta` keys (`logicalCwd`,
  `activeSandboxId`, `usage`, `attachedSkills`, `turnRunId`, `turnStatus`) through the same
  `readEnvelope`/`upsertEnvelope` read-copy-override contract — a worker PUT never wipes
  `persona`/`selectedModel`/`title`. The durable model context (**truncated
  `response.messages`**, aligned #549) is persisted as its **own Blob transcript object**,
  never in the 1 MiB envelope `meta`.
