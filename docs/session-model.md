...
and (phase 2 #517) `attachedSlugs` into `meta.attachedSkills`, and (plan #616 / source #610)
`selectedModel` into `meta.selectedModel` (shared client-safe predicates; a host-absolute cwd /
non-Redis-safe id / non-printable-ASCII model is dropped to unset),
