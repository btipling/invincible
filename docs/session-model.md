...
| `reasoningEffort` | **Optional** selected reasoning-effort token (Gateway language-model wire value, e.g. `low` / `high` / `xhigh`). **Session-owned** (plan #898): synced as reserved `meta.reasoningEffort`. Restore-by-value after the host pushes this model's joined list (protocol **v23**). Joined options adapt onto the language-model wire (`max` → `xhigh`, never published as `max`). Poison / not-in-list / unset → `defaultEffortFromOptions` (never auto `max` / `xhigh`). A stored `max` restores as `xhigh` when that token is listed. `sanitizeReasoningEffort` (`^[a-z0-9_-]{1,32}$`) drops charset poison to unset — never 400. Submit reads live `getSelectedReasoning()` |
