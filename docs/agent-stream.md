...
| Auto-continue per give-up | **1** (`AUTO_CONTINUE_PER_GIVE_UP`) | Host one-shot after classified recoverable bookkeeping give-up. Queue / pending submit / running envelope win. Host SSE retry 5 / 1 after paint is **unchanged** (same POST). |
...
| Recoverable bookkeeping auto-continue | `lib/turnRecoverable.ts` (client-safe classifier), `lib/harnessCanAutoContinue.ts`, `app/harness/HarnessHost.tsx` (one `'continue'` POST, `skipUserAppend`) — **not** same-POST retry |
