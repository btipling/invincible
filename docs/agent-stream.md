...
| Effort (harness picker) | In-canvas menu next to the model (protocol v23). Hidden when neither models.dev `vercel.models[id]` nor Gateway published values for this id. Live pick is `POST { reasoning }`. `max` is listable if the joined catalog lists it; never the restore/default. Unset (NEVER_AUTO-only lists) shows `effort` until the operator commits — the trigger must not look like `max` is already selected. |
...
| `GET /api/models` `reasoningOptions` | Per-model joined catalog: models.dev `vercel.models[id]` overlay (non-empty list wins), then Gateway `type: effort` values. Empty array when both miss. |
