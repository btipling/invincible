...
These markers are **not** folded as tools into the next prompt. Error turn-end lines **are** folded as `Error: …` so Continue sees them. Hitting the step cap also runs one tools-off wrap-up round this turn (`Error: step budget exhausted` in the model messages) so the agent can say what it completed before the canvas Error line. Cap always lands with open tool-calls; those pairs are closed with a skipped tool result first so the provider will accept the wrap-up user message.
