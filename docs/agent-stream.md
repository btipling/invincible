...
| Request `reasoning` | Optional body field on `POST /api/turns` and `POST /api/agent` (`^[a-z0-9_-]{1,32}$`). Wins over env when the token is on the language-model wire; off-wire tokens (`max`) are ignored and fall through to the catalog default. |
| `AGENT_REASONING` | Ops override only: `provider-default` \| `none` \| `low` \| `medium` \| `high`. Not the operator UI. Do not set `max` here. |
| `GET /api/models` `reasoningOptions` | Per-model joined catalog: Gateway `type: effort` values when published; models.dev `vercel.models[id]` fills when Gateway omitted or empty. Both sides drop tokens not on the language-model wire (`max` today). Empty array when both miss. |
