...
| `usage` | `usage` | **Live mid-stream** provider token summary from a `finish-step`/`finish` part. The host folds the context slot immediately; `done.usage` is the final reconcile. Absent when the part carried no usable counts — never a clear/flicker |
