...
| Stream write retries | **3** extra (`STREAM_WRITE_RETRY_ATTEMPTS`; 4 attempts total) | In-process retry of a Workflows user-stream PUT 5xx / 429 / timeout, then drop. Backoff 250 ms base, 4 s cap. Held writer latches dead so later tokens no-op. AbortError is never retried or latched. |
...
| Durable stream write | `lib/workflows/turnSseWrite.ts` (retry then drop writer I/O; one held writer per model/tool burst; sparse `writeOnDefaultStream` for loop `done` / `error`) |
