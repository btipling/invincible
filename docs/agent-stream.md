...
| Durable-turn wall clock | `lib/sessionCloudCaps.ts` (`TURN_WALL_CLOCK_MAX_MS` + TTL/probe seams), `lib/workflows/turnWorkflow.ts` (`deadlineAt` from `getWorkflowMetadata().workflowStartedAt`), `lib/workflows/modelGenerateStep.ts` + `lib/agent/generateOneRound.ts` (deadline `AbortSignal`, `'wall_clock'` code on deadline abort), `lib/workflows/toolExecuteStep.ts` (whole-batch deadline gate + `'wall_clock'`), `lib/workflows/turnLog.ts` (`invincible.turn.loop`) |
