...
| `GET /api/models` `reasoningOptions` | Per-model joined catalog: Gateway `type: effort` values when published; models.dev `vercel.models[id]` fills when Gateway omitted or empty. Tokens not in the Gateway language-model wire enum (`max`) are dropped. Empty array when both miss. |
