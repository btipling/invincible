...
| Recoverable bookkeeping auto-continue | `lib/turnRecoverable.ts` (client-safe classifier; `_` folds to space so `code` and `error` match), `lib/harnessCanAutoContinue.ts`, `app/harness/HarnessHost.tsx` (one `'continue'` POST, `skipUserAppend`, mint-bind flag migrate) — **not** same-POST retry |
