...
| Workflow run/step (backend-agents) | **Vercel backend** | Server-only (`lib/workflows/*` + the **turn Workflow** `lib/agent/turnWorkflow.ts`, `"use workflow"` orchestration via the `workflow` SDK + `withWorkflow` in `next.config.js`), authed smoke (`app/api/workflows/smoke`), Vercel Workflows platform (Functions + Queues, dashboard-visible Observability). Never Wasm/DOM; never the tab-owned `/api/agent` as the smoke path. Verify via GHA `workflows-smoke`. **Slice E (#791): the Workflow is the turn owner for the durable E path** — `POST /api/turns` starts `runTurnWorkflow` (one prompt = one run; re-resolves grants/BYOK/sandbox/MCP/http per step; worker persists Blob + envelope LWW; emits the same `AgentStreamEvent`s). `/api/agent` is **not** cut over yet — F owns the open-UI host ("attach") path |
