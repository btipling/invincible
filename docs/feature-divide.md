...
| `POST /api/turns` | **Vercel backend** | Production harness turn (Workflows + SSE). Host `runHarnessTurn` default. Same tool surface as `/api/agent`. Plan #944: the seeded `priorMessages` (when a bound `modelMessagesPointer` exists) is trimmed to the model **window − reserve** token budget (+ row/byte rails) at the route boundary (`trimModelMessagesToBudget`); the window resolves from the joined catalog (`getJoinedWindowMap`), conservative default when unpublished |
...
| Context compaction (durable-turn summary checkpoint) | **Vercel backend** | When the pre-trim seed exceeds the selected model's fold budget, the backend cuts at a user-turn boundary (re-paired, tool calls never split from their results), runs **one tools-off summarizer step** server-side in the durable Workflow (same model, BYOK re-resolved in-step; no tools), and persists the checkpoint `{summary, filesTouched, retainedTail}` as its own session-bound Blob object behind `meta.compactionPointer`. The **DOM host stays a thin submit/bridge shell** (no summarizer, no new surface); the **Wasm canvas** stays the primary transcript/composer — it paints nothing for compaction (the summary row is server-side seed only) and never runs the summary. The next turn prefers the honesty-prefixed live model-messages warehouse, then the checkpoint, then ordinary model messages, then the legacy `promptHistory` sidecar — every pointer session-bound and re-validated on read (see [session-model.md](session-model.md) · Compaction). Wasm/DOM never touch the checkpoint Blob or Redis directly |
