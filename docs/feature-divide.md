...
| **Workspace status bar** (sandbox · cwd · git · context) | **Wasm-primary** (header slot pack, protocol v13) | Product truth is in-canvas: a right-aligned pack of status slots painted by the Wasm header band from bridge state. The **DOM host mirrors only** (never a competing status panel). Host folds session state — sandbox (`activeSandboxId`) + `cwd` — into the bridge status-slot store after hydrate/restore and after **every** agent turn (success **and** fail: a 403-clear or a committed `change_dir` on a cancelled/timed-out turn repaints the pack — PR #543; host-ellipsized to the byte cap before the wire). Git (Phase 2) + context/usage (Phase 3) are reserved slot indices, not shipped here |
