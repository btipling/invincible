...
| Reasoning effort UI (status-bar menu) | **Wasm** | Status bar line 1, next to the model menu (protocol **v23**). Hidden when `GET /api/models.reasoningOptions` for the current id is empty. Host pushes that model's joined catalog values (Gateway list when published; models.dev `vercel.models[id]` fills holes; tokens not in the Gateway language-model enum — notably `max` — are dropped). The operator pick rides `meta.reasoningEffort` (reserved key, drop-to-unset). Host computes the default via `defaultEffortFromOptions` (never auto `max`); stored `max` coerces to `xhigh`/`high`. Wasm only paints + raises pending on click. Submit reads live `getSelectedReasoning()` into `POST /api/turns` `{ reasoning }` |
