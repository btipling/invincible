...
| Reasoning effort UI (status-bar menu) | **Wasm** | Status bar line 1, next to the model menu (protocol **v23**). Hidden when `GET /api/models.reasoningOptions` for the current id is empty. Host pushes that model's joined catalog values (Gateway list when published; models.dev `vercel.models[id]` fills holes; not stuffed into the id catalog). The operator pick rides `meta.reasoningEffort` (reserved key, drop-to-unset). Host computes the default via `defaultEffortFromOptions` (never auto `max`); Wasm only paints + raises pending on click. Submit reads live `getSelectedReasoning()` into `POST /api/turns` `{ reasoning }` |
