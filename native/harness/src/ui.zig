...
pub fn frame() !void {
    ...
    var typed: []const u8 = state.prompt_buf[0..0];
    {
        ...
        // it marks the chord handled before the textEntry is built, so the
        // widget never injects a stray newline for the submit stroke. Submit is
        // requested via `state.request_submit` (consumed after `te.deinit`).
        keymap_dispatch.dispatch(.{ .history = &historyApply });
        {
            ...
            // before the textEntry was built. After `te.deinit` the live prompt
