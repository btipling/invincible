...
//!
//! Tests cover: head position, cyclic distance, color-step mapping, scalar count,
//! empty/single-char edges, UTF-8 multi-byte safety, and full-cycle ramp coverage.
...
test "phase 0: all scalars map to colorStep 0 (solid warm_accent)" {
    // Plan #655 Goal 4 + DoD row 8: reduced motion / old host → phase 0 →
    // every glyph at ramp[0]. The cyclic distance formula puts only i=0 at
    // ramp[0]; every other scalar lands at ramp[3] (warm_surface, near-black
    // on teal_bg). The paint function fast-paths phase==0 with a single
    // addText — this test locks the domain logic that the fast-path keeps
    // honest, and if the fast-path is ever removed it fails here.
...
test "colorStep: head position at i=0 for 18-char text is step 0 (smoke)" {
    // head = 0/3 = 0.0: cyclicDistance(0, 0.0, 18) = 0 → step 0
...
test "colorStep: distance 0 → step 0 (head)" {
    try t.expectEqual(@as(usize, 0), text_wave.colorStep(0.0, 10));
}

test "colorStep: distance 5 on N=10 → step 3 (furthest = N/2)" {
    try t.expectEqual(@as(usize, 3), text_wave.colorStep(5.0, 10));
}
...
test "colorStep: all 4 stops exercised across full distance range on N=20" {
    // N=20, max distance = 10. Map dist 0..10 to steps 0..3 via dist*4/10.
    // dist 0  → 0*4/10 = 0.0  → step 0
    // dist 2  → 2*4/10 = 0.8  → step 0
    // dist 3  → 3*4/10 = 1.2  → step 1
    // dist 5  → 5*4/10 = 2.0  → step 2
    // dist 8  → 8*4/10 = 3.2  → step 3
    // dist 10 → 10*4/10 = 4.0 → min(4,3) = 3
    try t.expectEqual(@as(usize, 0), text_wave.colorStep(0.0, 20));
    try t.expectEqual(@as(usize, 0), text_wave.colorStep(2.0, 20));
    try t.expectEqual(@as(usize, 1), text_wave.colorStep(3.0, 20));
    try t.expectEqual(@as(usize, 2), text_wave.colorStep(5.0, 20));
    try t.expectEqual(@as(usize, 3), text_wave.colorStep(8.0, 20));
    try t.expectEqual(@as(usize, 3), text_wave.colorStep(10.0, 20));
}

test "colorStep: N=2 (two chars) — head and opposite" {
    // N=2, max cyclic dist = 1. denom = 1.0.
    // dist 0 → 0*4/1 = 0 → step 0
    // dist 1 → 1*4/1 = 4 → min(4,3) = 3
    try t.expectEqual(@as(usize, 0), text_wave.colorStep(0.0, 2));
    try t.expectEqual(@as(usize, 3), text_wave.colorStep(1.0, 2));
}
