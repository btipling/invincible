...
| **Model catalog** | Host fetches `GET /api/models` after load and **pushes** ids into Wasm; selection (cycle / label) stays in the canvas UI. Per-model `reasoningOptions` are pushed separately (v23) for the selected id — never stuffed into the id catalog |
